...
class add_technical:
    ...
    def lag_make(self, df, serries_columns, lag_span=np.arange(1, 11)):
        """
        ラグ特徴量作成関数 lag_spanでラグ数設定
        :param df 株価のdataframe
        :param serries_columns ラグ特徴量を生成したいcolumn名のlist
        :param lag_span ラグ数のarray 初期 1~10
        :return ファクターを追加したdf
        """
        for serries_column in serries_columns:
            for span in lag_span:
                df[f"{serries_column}_lag{span}"] = df[f"{serries_column}"].shift(span)
        return df
